...
In this two conditions, tasks schedule is done by \textsl{Parent}. And launching kernel is a cost work. If just one or a few blocks in \textsl{Child} is overloaded, it is obviously that the execution should not return to \textsl{Parent}. Instead, inside the \textsl{Child}, it should also have a \textsl{Child level inter blocks} task schedule. That, combined with the \textsl{Parent} level task schedule, we build the two-level task schedule. It can be shown in \ref{}.
...
Then we define a \textsl{PathRecordNodeList} array in each block that stored in Shared Memory (called \textsl{S_PRN_List}) and one more array in Global Memory (called \textsl{G_PRN_List}. On a whole, during the path generation, threads will update the path in corresponding node in Shared Memory following the inherit rules mentioned. And the updating of path in Global Memory happens when the overload happens in order to fit the dynamic tasks schedule design previously mentioned. In details, take \ref{} as an example:

\begin{figure}
	\vspace{2.5cm}
	\caption{}
	\label{fig:pathrecexplain}
\end{figure}

Based on \ref{}, the steps to do the path recording are as follows, all these steps are executed after the \textsl{IfReachSCC\&ACC} detection:

\begin{itemize}
	\item{when a node is being visited, update the corresponding PathRecordNode in the \textsl{S_PRN_List} by push itself to the path} 
...
Another problem is the storage space problem, in worst case, the structure our algorithm work on is a binary tree, then totally cost around 8KB. 
